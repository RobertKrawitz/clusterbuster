...
**Phase 1 (workload-options test harness):** The regression driver under [`tests/workload-options/`](tests/workload-options/) holds `cases.yaml`, reports, wrappers, and the Python package [`workload_options/`](tests/workload-options/workload_options/). The `.sh` scripts are thin wrappers; you can run `PYTHONPATH=tests/workload-options python3 -m workload_options` from the repo root (or `tests/workload-options/run_workload_option_tests.py`, which sets the path) with the same flags as before. See that directory’s README for details. Offline checks: `pip install -e .` (for reporting deps) and `pytest tests/test_workload_options.py` (pytest adds `lib/` and `tests/workload-options/` on the path via `pyproject.toml`).
