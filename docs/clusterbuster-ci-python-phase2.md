...
**Structured result (`ClusterbusterCISuiteResult`) deferral:** A frozen **`ClusterbusterCISuiteResult`**, **`run_perf_ci_suite() -> ClusterbusterCISuiteResult`**, exit-code table **2/3**, and **single-writer** JSON built from that type are **out of scope for Phase 2 close** per maintainer policy. Until a later milestone, **`ClusterbusterCISuite.run()`** and **`run_perf_ci_suite()`** return **`int`**; **`write_ci_results_json`** remains the writer for **`clusterbuster-ci-results.json`**.
