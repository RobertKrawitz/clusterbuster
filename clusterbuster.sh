...
cat 1>&2 <<'EOF'
*********************************************************************
* WARNING: clusterbuster.sh is DEPRECATED.                          *
*                                                                   *
* This bash version has been superseded by the new Python-based     *
* 'clusterbuster' driver.  clusterbuster.sh will not receive        *
* further updates and will be removed in a future release.          *
*                                                                   *
* Please use './clusterbuster' (the Python version) instead.        *
*********************************************************************
EOF
...
declare -r default_common_workdir=/var/tmp/clusterbuster
declare common_workdir=$default_common_workdir
declare -i report=0
...
# Sync pod always uses clusterbuster_base_image (or CLUSTERBUSTER_SYNC_IMAGE / --sync_pod_image).
# It never uses workload container_image. Requests only (no limits) on the sync pod.
# Sync always uses the Kubernetes pod network: the sync Pod is not hostNetwork; listeners bind
# on the pod IP. Workload pods connect over the CNI; VM guests reach sync via the virt-launcher
# pod (still pod network egress). NetworkPolicy/mesh must allow that path.
declare clusterbuster_base_image='quay.io/rkrawitz/clusterbuster-base:latest'
declare sync_pod_image_override=
declare cb_sync_pod_memory_request=512Mi
declare cb_sync_pod_cpu_request=2
...
declare container_image="$clusterbuster_base_image"
...
declare -i __virtctl_use_type_and_name=0
...
$(print_workloads_supporting_reporting '                        - ')
    ...
    Pod Options:
       --container_image=<image>
                        ...
                        clusterbuster_base_image). Does not apply to "pause" workloads.
                        The sync pod uses clusterbuster_base_image (defaults are fine;
                        CLUSTERBUSTER_SYNC_IMAGE or --sync_pod_image only if you must).
...
function process_option() {
	...
	clusterbusterbaseimage)	    clusterbuster_base_image=$optvalue		;;
	syncpodimage)		    sync_pod_image_override=$optvalue		;;
...
function has_writable_emptydir_workdir() {
    local volspec
    local -a args
    for volspec in "${volumes[@]}" ; do
	IFS=: read -ra args <<< "$volspec"
	if [[ ${args[2]} = "$common_workdir" ]] ; then
	    return 0
	fi
    done
    return 1
...
function requires_writable_workdir() {
    [[ -z "$runtime_class" && $common_workdir = "$default_common_workdir" &&
	   $create_pods_privileged -ne 0 ]] &&
	call_api -w "$requested_workload" requires_writable_workdir &&
	! has_writable_emptydir_workdir
}
...
# Architecture of the first schedulable worker (or local uname if cluster not queryable).
function cb_cluster_worker_architecture() {
    local a=
    if [[ -n "${OC:-}" ]] ; then
	a=$("$OC" get nodes -l 'node-role.kubernetes.io/worker=' -o jsonpath='{.items[0].status.nodeInfo.architecture}' 2>/dev/null) || true
	[[ -z "$a" ]] && a=$("$OC" get nodes -o jsonpath='{.items[0].status.nodeInfo.architecture}' 2>/dev/null) || true
    fi
    echo "${a:-$(uname -m)}"
}
################################################################
...
#
# As of virtctl 1.6, virtctls require specifying the remote as
# [user@]vm/[vm].[namespace]:[file].  Again, it's not clear how to determine that
# except for parsing the help message.  This appears to have been introduced in
# PR #13918, #13938, and #13939.
function _VIRTCTL_BASE() {
	...
	if [[ $__virtctl_local_ssh = UNKNOWN ]] ; then
	    ...
	    if virtctl ssh --help < /dev/null 2>&1 |grep -q -E -e '@vmi?/testvm' ; then
		__virtctl_use_type_and_name=1
	    fi
	fi
	local -a args=()
	local arg
	for arg in "$@" ; do
	    if ((__virtctl_use_type_and_name)) && [[ $arg =~ ([-_[:alnum:]]+@)?([-[:alnum:]]+)\.([-[:alnum:]]+):(.*) ]] ; then
		local user=${BASH_REMATCH[1]}
		local name=${BASH_REMATCH[2]}
		local namespace=${BASH_REMATCH[3]}
		local file=${BASH_REMATCH[4]}
		args+=("${user}vm/${name}/${namespace}:${file}")
	    else
		args+=("$arg")
	    fi
	done
	"$VIRTCTL" "$subcmd" ${__virtctl_local_ssh:+"$__virtctl_local_ssh"} "${args[@]}"
    else
	"$VIRTCTL" "$subcmd" "$@"
    fi
}
...
function create_sync_service() {
	...
	if [[ $namespace = "${namespaces_to_create[0]:-}" ]] ; then
	    # Selector must match sync pod labels. Using name: ${basename}-sync-sync is wrong: standard_labels_yaml
	    # for the sync pod sets name to sync-sync-${basename}-sync (workload/namespace both "sync"), so the
	    # Service had no endpoints and workloads could not resolve or reach the sync pod.
	    create_service -W -h -k "${basename}-sync-sync" -v "true" "$sync_namespace" "${sync_namespace}-sync" "$sync_port" "$sync_ns_port"
    ...
    cat <<EOF
{
 ...
 "metadata": {
  ...
  "options": {
   ...
   "clusterbuster_base_image": "$clusterbuster_base_image",
   "sync_pod_image": "${sync_pod_image_override:-${CLUSTERBUSTER_SYNC_IMAGE:-$clusterbuster_base_image}}",
...
function _extract_metrics() {
	...
	cat <<EOF
"metrics": $("${__libdir__}/prom-extract" --indent= --define "namespace_re=${basename}-.*" -m "$metrics_file" -s "$metrics_interval" --metrics-only --start_time="$((prometheus_starting_timestamp - metrics_epoch))" --post-settling-time="$((metrics_epoch*2))"),
EOF
    fi
}
...
function get_sync_logs() {
	...
	if [[ -n "$artifactdir" ]] ; then
	    ...
	    local final_report_file="$artifactdir/clusterbuster-report.json"
	    if ((compress_report)) ; then final_report_file+=".gz"; fi
	    mv "$artifactdir/clusterbuster-report.tmp.json" "$artifactdir/clusterbuster-report.json" &&
		( ((! "$compress_report")) || gzip -f "$artifactdir/clusterbuster-report.json" ) &&
		print_report < "$final_report_file"
	    return $status
	else
	    trap '' TERM
...
# Populate global array cb_pod_flags with --cb-* tokens for workload/drop-cache containers.
# Called from create_standard_containers / VM runcmd before each *_arglist invocation; workloads
# only consume "${cb_pod_flags[@]}" and no longer parse the 14 sync/drop-cache positional values.
function cb_pod_client_flags_array() {
    if (($# != 14)) ; then
	fatal "cb_pod_client_flags_array requires 14 arguments, got $#"
    fi
    cb_pod_flags=(
	"--cb-sync-nonce=$1"
	"--cb-namespace=$2"
	...
	"--cb-sync-port=$9"
	"--cb-sync-ns-port=${10}"
	"--cb-sync-watchdog-port=${11}"
	"--cb-sync-watchdog-timeout=${12}"
	"--cb-drop-cache-host=${13}"
    )
    if [[ -n "${14}" ]] ; then
	cb_pod_flags+=("--cb-drop-cache-port=${14}")
    fi
}
...
$(if [[ -n "${arglist_function}" ]] ; then
     cb_pod_client_flags_array "$sync_nonce" "$namespace" "c$container" "$basetime" "$baseoffset" "$(ts)" "$exit_at_end" "$sync_service" \
     "$sync_port_num" "$sync_ns_port_num" "$sync_watchdog_port_num" "$sync_watchdog_timeout" \
     "$drop_cache_service" "$drop_cache_port_num"
     "${arglist_function}" "${system_configmap_mount_dir}/" "$@" "$container" -- | indent 2 ; fi)
...
EOF
	...
	local volspec
	local -i emptyvolid=0
	if requires_writable_workdir ; then
	    name="cbemptydir$((emptyvolid++))"
	    cat <<EOF
- name: $name
  mountPath: "${common_workdir}"
EOF
	fi
	for volspec in "${volumes[@]}" ; do
	    local -a args
...
EOF
	_OC label namespace --overwrite "$namespace" "pod-security.kubernetes.io/enforce=$policy" "pod-security.kubernetes.io/audit=$policy" "pod-security.kubernetes.io/warn=$policy" 2>&1 | report_if_desired
...
function vm_volume_devices_yaml() {
	...
	for arg in "${args[@]}" ; do
		...
		bus)   bus="$value"   			  ;;
		cache) cache="$value" 			  ;;
		dedicatediothread) dedicatedIOThread=true ;;
		*)		      			  ;;
	    esac
	done
...
- name: $name
  serial: $name
${dedicatedIOThread:+  dedicatedIOThread: $dedicatedIOThread}
  disk:
    bus: $bus
${cache:+    cache: $cache}
EOF
...
$(indent 16 _setup_sysctls)
     ...
     cb_pod_client_flags_array "$sync_nonce" "$namespace" "c0" "$basetime" "$baseoffset" "$(ts)" "$exit_at_end" "$sync_service" \
     "$sync_port_num" "$sync_ns_port_num" "$sync_watchdog_port_num" "$sync_watchdog_timeout" \
     "$drop_cache_service" "$drop_cache_port_num"
     ARGLIST_FORMAT=vm "${arglist_function}" "${system_configmap_mount_dir}/" "$@" "0" -- ; fi)"
EOF
    done
}
...
function create_container_sync() {
    ...
    local _sync_pod_img="${sync_pod_image_override:-${CLUSTERBUSTER_SYNC_IMAGE:-$clusterbuster_base_image}}"
    cat <<EOF
- name: ${namespace}
${image_pull_policy:+  imagePullPolicy: $image_pull_policy}
  image: "$_sync_pod_img"
...
$(indent 2 standard_environment)
  ...
  - "--cb-sync-nonce=$sync_nonce"
  - "--cb-sync-file=$sync_flag_file"
  - "--cb-error-file=$sync_error_file"
  - "--cb-controller-timestamp-file=$controller_timestamp_file"
  - "--cb-predelay=$predelay"
  - "--cb-postdelay=$postdelay"
  - "--cb-step-interval=$workload_step_interval"
  - "--cb-listen-port=$sync_port"
  - "--cb-ns-port=$sync_ns_port"
  - "--cb-watchdog-port=$sync_watchdog_port_num"
  - "--cb-watchdog-timeout=$sync_watchdog_timeout"
  - "--cb-expected-clients=$expected_clients"
  - "--cb-initial-expected-clients=$initial_expected_clients"
$(indent 2 volume_mounts_yaml -V "$namespace" 0 0)
  resources:
    requests:
      memory: "${cb_sync_pod_memory_request}"
      cpu: "${cb_sync_pod_cpu_request}"
...
$(indent 2 standard_environment)
  ...
  - "--cb-baseoffset=$baseoffset"
  - "--cb-crtime=$(ts)"
  - "--cb-exit-at-end=0"
  - "--cb-sync-host=$sync_service"
  - "--cb-sync-port=$sync_port_num"
  - "--cb-sync-ns-port=$sync_ns_port"
  - "--cb-sync-watchdog-port=$sync_watchdog_port_num"
  - "--cb-sync-watchdog-timeout=$sync_watchdog_timeout"
  - "--cb-drop-cache-host=none"
  - "--cb-drop-cache-port=$drop_cache_port"
$(indent 2 volume_mounts_yaml -V "$namespace" 0 0)
...
function _do_cleanup_1() {
    ...
    local ignf='--ignore-not-found=true'
    if ((force)) ; then
	...
	exec 2>/dev/null
	# shellcheck disable=SC2086
	____OC delete $objects_to_clean -l "${basename}-sync" $ignf $ltimeout
	# shellcheck disable=SC2086
	____OC delete $objects_to_clean -l "$basename" $ignf $ltimeout
    elif ((report_object_creation)) ; then
	# shellcheck disable=SC2086
	__OC delete $objects_to_clean -l "${basename}-sync" $ignf $ltimeout 1>&2
	# shellcheck disable=SC2086
	__OC delete $objects_to_clean -l "${basename}" $ignf $ltimeout 1>&2
    else
	# shellcheck disable=SC2086
	__OC delete $objects_to_clean -l "${basename}-sync" $ignf $ltimeout 2>&1 |grep 'DEBUG kubectl:' 1>&2
	# shellcheck disable=SC2086
	__OC delete $objects_to_clean -l "$basename" $ignf $ltimeout 2>&1 |grep 'DEBUG kubectl:' 1>&2
    fi
}
...
function _do_cleanup() {
	...
	if [[ -n "$force_cleanup_timeout" ]] ; then
	    ...
	    if [[ $deployment_type = vm ]] ; then
		__OC delete all -A -l "${basename}-sync" --force --grace-period=0 --ignore-not-found=true 2>&1 | timestamp 1>&2
	    fi
	    __OC delete all -A -l "$basename" --force --grace-period=0 --ignore-not-found=true 2>&1 | timestamp 1>&2
	    if ((use_namespaces && remove_namespaces)) ; then
		__OC delete namespace -l "$basename" --force --grace-period=0 --ignore-not-found=true 2>&1 | timestamp 1>&2
	    fi
	fi
    fi
}
...
function run_clusterbuster_2() {
    ...
    do_logging || return 1
    if ((report_object_creation)) && (! get_run_failed || ((! status)) ) ; then
...
function finalize_expanded_command_line_extra_args() {
    [[ ${#extra_args[@]} -eq 0 ]] && return 0
    if declare -F "${requested_workload}_finalize_extra_cli_args" &>/dev/null; then
	"${requested_workload}_finalize_extra_cli_args"
    else
	processed_options+=("${extra_args[@]}")
    fi
}
...
function validate_options() {
	...
	*)
    ...
    # KubeVirt aarch64 VMs often fail at UEFI/boot with current containerdisk images; disable until fixed.
    if [[ $deployment_type = vm && ${CB_ALLOW_VM_AARCH64:-0} != "1" ]] ; then
	local warch
	warch=$(cb_cluster_worker_architecture)
	case "$warch" in
	aarch64|arm64)
	    fatal "VM deployment is disabled on aarch64 clusters (KubeVirt/UEFI/containerdisk). Use pods or kata, or set CB_ALLOW_VM_AARCH64=1 to override after debugging."
	    ;;
	esac
    fi
...
while getopts ":B:Eef:Hhno:P:w:QqvNz-:" opt ; do
    case "$opt" in
	...
	z) process_option "compressreport=1"	 ;;
...
finalize_expanded_command_line_extra_args
