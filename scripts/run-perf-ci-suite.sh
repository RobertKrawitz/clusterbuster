...
declare __force_pull_clusterbuster_image__="${__libdir__}/force-pull-clusterbuster-image"
...
function list_profiles() {
	...
	declare -A seen=()
	local f base
	for f in "${__profiledir__}"/*.yaml "${__profiledir__}"/*.yml "${__profiledir__}"/*.profile ; do
	    [[ -e "$f" ]] || continue
	    base=${f##*/}
	    base=${base%%.yaml}
	    base=${base%%.yml}
...
function process_profile() {
    ...
    local profile=
    if [[ $profile_arg != *'/'* ]] ; then
	if [[ -f "${__profiledir__}/${profile_arg}.yaml" ]] ; then
	    profile="${__profiledir__}/${profile_arg}.yaml"
	elif [[ -f "${__profiledir__}/${profile_arg}.yml" ]] ; then
	    profile="${__profiledir__}/${profile_arg}.yml"
	elif [[ -f "${__profiledir__}/${profile_arg}.profile" ]] ; then
	    profile="${__profiledir__}/${profile_arg}.profile"
	else
	    profile="${__profiledir__}/${profile_arg}.yaml"
	fi
	profile_in_dir=1
	...
	case "$profile" in
	    *.yaml|*.yml)
		local tf line
		tf=$(mktemp "${TMPDIR:-/tmp}/cb-profile.XXXXXX") || fatal "mktemp failed for profile parse"
		if ! PYTHONPATH="${__topdir__}/lib" "${PYTHON:-python3}" -m clusterbuster.ci.profile_yaml "$profile" >"$tf" ; then
		    rm -f "$tf"
		    fatal "Failed to parse YAML profile $profile (is PyYAML installed?)"
		fi
	...
	fatal "Cannot find profile $profile_arg in $__profiledir__"
    else
	fatal "Cannot find profile $profile_arg"
    fi
}
...
$(_document_workloads)
    ...
    General options:
        ...
        --profile=profile       Which profile to use (basename of a .yaml/.yml
                                file under $__profiledir__, or legacy .profile).
...
function process_option() {
	...
	workloads) readarray -t workloads <<< "$(parse_optvalues "$optvalue")"		;;
	*) extra_args+=("$option")							;;
    esac
}
...
if ((! debugonly)) ; then
    ...
    # Skip vm jobs on aarch64 clusters (same policy as clusterbuster CB_ALLOW_VM_AARCH64).
    if [[ "${CB_ALLOW_VM_AARCH64:-0}" != "1" && -n "${OC:-}" ]] ; then
	cluster_arch=$("${OC}" get nodes -l 'node-role.kubernetes.io/worker=' -o jsonpath='{.items[0].status.nodeInfo.architecture}' 2>/dev/null) || true
	[[ -z "$cluster_arch" ]] && cluster_arch=$("${OC}" get nodes -o jsonpath='{.items[0].status.nodeInfo.architecture}' 2>/dev/null) || true
	if [[ "$cluster_arch" = aarch64 || "$cluster_arch" = arm64 ]] ; then
	    filtered_rc=()
	    for runtimeclass in "${runtimeclasses[@]}" ; do
		if [[ "$runtimeclass" = vm ]] ; then
		    echo "run-perf-ci-suite: skipping runtimeclass vm on aarch64 cluster (set CB_ALLOW_VM_AARCH64=1 to force)" 1>&2
		    continue
		fi
		filtered_rc+=("$runtimeclass")
	    done
	    runtimeclasses=("${filtered_rc[@]}")
	fi
    fi
