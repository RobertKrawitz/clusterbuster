...
def test_all_shipped_yaml_profiles_load() -> None:
    root = Path(__file__).resolve().parents[1]
    prof_dir = root / "lib" / "CI" / "profiles"
    for path in sorted(prof_dir.glob("*.yaml")):
        lines = load_yaml_profile(path)
        assert isinstance(lines, list)
        assert lines, f"{path.name} produced no option lines"


def test_resolve_profile_name_func_ci() -> None:
    root = Path(__file__).resolve().parents[1]
    p = resolve_profile_path("func_ci", root / "lib" / "CI" / "profiles")
    assert p.name == "func_ci.yaml"
