...
def emit_loader_tsv_lines(cases_yaml_path: str, deployment_targets: str) -> Iterator[str]:
    """Legacy TSV stream for debugging; columns match the old load-cases-yaml helper."""
...
def main_loader() -> None:
    """CLI: same as legacy load-cases-yaml."""
    import sys

    if len(sys.argv) < 3:
        print("usage: load-cases-yaml <cases.yaml> <deployment-targets>", file=sys.stderr)
        raise SystemExit(2)
    path = sys.argv[1]
    targets = sys.argv[2]
    try:
        ...
        print(f"load-cases-yaml: {e}", file=sys.stderr)
