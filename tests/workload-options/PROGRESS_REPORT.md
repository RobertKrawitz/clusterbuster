...
- Prefer **command-line options** (`./run-workload-option-tests.sh --help`); options are flags only (no `CB_TEST_*` environment fallbacks).

## Dry run

- **Command**: `./tests/workload-options/run-workload-option-tests.sh --mode dry` (dry is the default).

## Live run on `scale`

- **Command**: `ssh scale 'cd rlk/clusterbuster && git pull && ./tests/workload-options/run-workload-option-tests.sh --mode live'`
