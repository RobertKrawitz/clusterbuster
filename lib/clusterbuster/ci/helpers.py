...
def compute_timeout(timeout: int, job_timeout: int) -> int:
    """Effective timeout: non-positive ``timeout`` uses ``job_timeout``; negative flips sign."""
    if timeout <= 0:
        timeout = job_timeout
    if timeout < 0:
        timeout = -timeout
...
def computeit(expr: str) -> int:
    """Integer result of a simple arithmetic expression.
...
def roundup_interval(base: int, interval: int) -> int:
    """Round up to a multiple of ``base`` (used by the files workload)."""
