...
"""CI-specific options (--ci-*) and scoped option parsing (bash parity)."""
...
class ParsedCiOption:
    """Output of :func:`parse_ci_option` (stripped names, as in bash)."""
...
def splitarg(value: str) -> str:
    """Comma to space (``splitarg`` from run-perf-ci-suite)."""
...
def check_ci_option(noptname: str, workload: str, runtime: str) -> bool:
    """``_check_ci_option`` from run-perf-ci-suite."""
...
def parse_ci_option(option: str, workload: str = "", runtime: str = "") -> ParsedCiOption | None:
    ...
    if not check_ci_option(po.noptname, workload, runtime):
        return None
    # Bash: echo "${noptname1/:*/} ${noptname/:*/} $optvalue"
...
def add_ci_argument_group(parser: argparse.ArgumentParser) -> argparse._ArgumentGroup:
    ...
    g.add_argument(
        ...
        default="",
        metavar="ARGS",
        help="Extra arguments for CI workloads (shell-quoted)",
    )
    g.add_argument(
        "--ci-clusterbuster-args",
        default="",
        metavar="ARGS",
        help="Extra arguments passed to clusterbuster (shell-quoted)",
    )
    g.add_argument(
        "--ci-job-delay",
        type=int,
