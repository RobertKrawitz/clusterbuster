    ...
    def run(self, suite: Any, default_job_runtime: int) -> None:
        for runtimeclass in suite.config.normalized_runtimeclasses():
            ...
            for options in specs:
                parts = options.split(":")
                if len(parts) < 6:
                    print(f"Unparsable hammerdb options {options!r}", flush=True)
                    continue
                ...
                if not (
                    ...
                    print(f"Invalid hammerdb options {options!r}", flush=True)
                    continue
                driver_l = driver.lower()
                if driver_l not in ("pg", "mariadb"):
                    print(f"Invalid driver {driver!r} in hammerdb options", flush=True)
                    continue
