...
from clusterbuster.ci.restart import is_report_dir, recover_uuid_from_artifact_root
...
def run_suite_with_orchestration(state: CISuiteState, workload_args: list[str]) -> int:
    ...
    if workload_args:
        wl = workload_args
    elif state.workloads:
        wl = list(state.workloads)
    else:
        wl = list(default_registry().keys())
    for w in wl:
    ...
    if not dry:
        ...
        # Match ``scripts/run-perf-ci-suite.sh``: restart → recover uuid from first job report;
        ...
        if state.restart:
            if art.exists():
                ...
                if recovered:
                    state.uuid = recovered
            if not art.exists():
                art.mkdir(parents=True, exist_ok=True)
        else:
            if art.exists() and is_report_dir(art):
                shutil.rmtree(art)
            if not art.exists():
                art.mkdir(parents=True, exist_ok=True)
    if not state.uuid:
