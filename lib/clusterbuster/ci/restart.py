...
"""Restart and resume: detect finished job trees and recover run UUID from prior artifacts."""
...
def is_report_dir(path: Path) -> bool:
    """True if the directory contains a finished ClusterBuster report (JSON or gzipped JSON)."""
    if not path.is_dir():
        return False
